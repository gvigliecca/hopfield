...
{} pasos.'.format(t_conv))
    ...
    df[str(neurons_list[k])] = m_mean_vals

df.to_csv('Data/data.csv', float_format='%.3f', index=False)

for k in range(len(parameters_list)):
    label = '{} neuronas'.format(neurons_list[k])
    plt.scatter(alpha_vals, df[str(neurons_list[k])], label=label)
...
plt.savefig('Figs/capacidad.pdf')
